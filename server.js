...
app.get('/api/stats', async (req, res) => {
  ...
  try {
    ...
    const avgWin = wins > 0
      ? trades.filter(t => (t.pnl_pct || 0) > 0).reduce((s, t) => s + (t.pnl_pct || 0) * (t.size_usdt || 0) / 100, 0) / wins
      : 0;
    const avgLoss = losses > 0
      ? Math.abs(trades.filter(t => (t.pnl_pct || 0) <= 0).reduce((s, t) => s + (t.pnl_pct || 0) * (t.size_usdt || 0) / 100, 0) / losses)
      : 0;
